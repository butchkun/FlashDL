...
if ($action === 'abort') {
  ...
  // best-effort cleanup
  if (is_dir($tmpDir)) {
    foreach (glob($tmpDir . DIRECTORY_SEPARATOR . 'chunk_*.part') ?: [] as $f) @unlink($f);
    @rmdir($tmpDir);
  }

  $pdo->prepare("UPDATE upload_sessions SET aborted_at=:t WHERE token=:t")
      ->execute([':t'=>date('c'), ':t'=>$token]);
...
if ($action === 'finalize_bundle') {
  ...
  // Remove assembled + chunks dirs (optionnel: purge plus tard)
  foreach (glob($tmpDir . DIRECTORY_SEPARATOR . 'assembled_*.bin') ?: [] as $a) @unlink($a);
  // (tu peux faire une purge différée, plus sûr)
