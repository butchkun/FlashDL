...
function parse_bigint($v): int {
  $s = trim((string)$v);
  if ($s === '') return 0;

  // Normalize common separators/spaces (including NBSP and narrow NBSP)
  $s = str_replace(["\xC2\xA0", "\xE2\x80\xAF", " ", "\t", "\r", "\n"], '', $s);

  // Scientific notation like "3.38e+10"
  if (preg_match('/^[0-9]+(?:\.[0-9]+)?e[+\-]?[0-9]+$/i', $s)) {
    $f = (float)$s;
    if (!is_finite($f) || $f < 0) return 0;
    return (int)round($f);
  }

  // Keep digits only (handles "33,800,000,000" or "33 800 000 000")
...
if ($action === 'init') {
  ...
  $totalBytes = parse_bigint($_POST['total_bytes'] ?? 0);
  $chunkSize  = parse_bigint($_POST['chunk_size'] ?? 0);
...
if ($action === 'init_bundle') {
  ...
  foreach ($manifest['files'] as $f) {
    $tb = parse_bigint($f['size'] ?? ($f['total_bytes'] ?? 0));
    if ($tb < 0) $tb = 0;
    $totalBytes += $tb;
  }
...
if ($action === 'finalize_bundle') {
  ...
  $zipBase = ($bundleName !== '') ? safe_filename($bundleName) : ('transfert_' . date('Ymd_His'));
  if ($zipBase === '' || $zipBase === '.' || $zipBase === '..') $zipBase = ('transfert_' . date('Ymd_His'));
