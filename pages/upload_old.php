...
testApiBtn && testApiBtn.addEventListener('click', async (e)=>{ e.preventDefault();
  try{
    const r = await apiCall({action:'ping'});
    showError('Réponse ping API', JSON.stringify(r, null, 2));
  }catch(err){
    const msg = (err && err.message) ? err.message : String(err);
    const stack = (err && err.stack) ? err.stack : '';
    showError('Ping API échoué', msg + (stack ? ('\n\n' + stack) : ''));
  }
});
